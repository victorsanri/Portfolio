...
class Celda1{
    ...
    public LinkedList<Celda1> getSucesores() {
        return sucesores;
    }

    public void insertarsucesor(Celda1 c){
        sucesores.add(c);
    }
}
...
class Nodo {
    ...
    private Nodo padre;
    private Celda1 actual;
    
    public Nodo (Celda1 actual){
        this.padre=null;
        this.actual=actual;
        this.coste=0;
        this.evaluacion=-1;
    ...
    public Celda1 getActual() {
        return actual;
    }

    public void setActual(Celda1 actual) {
        this.actual = actual;
    }   
}
...
class MatrizDinamica1 {
    private Celda1[][] matriz;
    private int tamx;
    private int tamy;
    
    public MatrizDinamica1(){
        tamx=5;
        tamy=5;
        matriz=new Celda1 [tamx][tamy];
    }
    
    public void set(int x,int y, Celda1 valor){
        if(x>=tamx){
            Celda1[][]aux=matriz;
            matriz= new Celda1[x+1][];
            System.arraycopy(aux,0,matriz,0,aux.length);
            for(int i=tamx;i<x+1;i++){
                matriz[i]=new Celda1[tamy];
            }
            tamx=x+1;
        }
...
public class M17C09b extends Mouse{
    private MatrizDinamica1 matriz;
    private int queso=0;
    ...
    private LinkedList<Integer> algoritmoAEstrella(Celda1 actual, int xQueso, int yQueso ){
        Comparator<Nodo> comparator = new MyComparator();
        TreeMap<String,Nodo> Tabla_A=new TreeMap<>();
        PriorityQueue<Nodo> abierta=new PriorityQueue<>(11,comparator);
        ...
        while(!abierta.isEmpty()){
            ...
            LinkedList<Celda1> sucesores= n.getActual().getSucesores();
            for(int i=0;i<sucesores.size();i++){
                if(Tabla_A.containsKey(sucesores.get(i).getX()+","+sucesores.get(i).getY())){
                    Nodo q=Tabla_A.get(sucesores.get(i).getX()+","+sucesores.get(i).getY());    
                    rectificar(q,n,1,Tabla_A); 
                    abierta=reordenar(abierta);
                } else {
                    Nodo q=new Nodo(sucesores.get(i));
                    q.setPadre(n);
                    q.setCoste(q.getPadre().getCoste()+1);
                    q.setCoste(n.getCoste()+1);
                    q.setEvaluacion(distanciaManhattan(q.getActual().getX(),q.getActual().getY(),xQueso,yQueso));
    ...
    private PriorityQueue<Nodo> reordenar (PriorityQueue<Nodo> cola){
        Comparator<Nodo> comparator = new MyComparator();
        PriorityQueue<Nodo> nuevo=new PriorityQueue<>(11,comparator);
        while(!cola.isEmpty()){
            nuevo.add(cola.poll());
        }
        return nuevo;
    ...
    private void rectificar(Nodo n,Nodo p, int costepn, TreeMap<String,Nodo> Tabla_A){
    ...
    private void RectificarLista(Nodo n, TreeMap<String,Nodo> Tabla_A){
        LinkedList<Celda1> lista=n.getActual().getSucesores();
        for(int i=0;i<lista.size();i++){
            if(Tabla_A.containsKey(lista.get(i).getX()+","+lista.get(i).getY())){
                Nodo q=Tabla_A.get(lista.get(i).getX()+","+lista.get(i).getY()); 
                rectificar(q,n,1,Tabla_A);
            }
        }
    }
    ...
    private LinkedList<Integer> calcularCamino(Nodo n, TreeMap<String,Nodo> Tabla_A ){
    ...
    private int distanciaManhattan(int x, int y, int xQueso, int yQueso){
        return Math.abs(x-xQueso)+Math.abs(y-yQueso);
    }
    
    
    //indica si la celda pasada como parametro es la del queso que buscamos
    private boolean objetivo(Celda1 celda, int xQueso, int yQueso){
    ...
    private int calcularDireccion(Celda1 origen, Celda1 destino){
