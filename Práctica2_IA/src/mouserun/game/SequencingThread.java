	...
	extends Thread
{
	...
	private final ArrayList<MouseRepresent> mouseInstances;
	private final HashMap<MouseRepresent, Integer> diseaseSymptomsCounter;
	...
	{
		mouseInstances = new ArrayList<>();
		diseaseSymptomsCounter = new HashMap<>();
	}
	...
	public void run()
	{
		...
		HashMap<MouseRepresent, Grid> movableMouse = new HashMap<>();
		HashMap<Long, ArrayList<MouseRepresent>> mouseTime = new HashMap<>(); 
		HashMap<MouseRepresent, Integer> mouseMove = new HashMap<>();
		...
		while (isAlive)
		{
			...
			ArrayList<MouseRepresent> retireList = new ArrayList<>();
                        mouseInstances.stream().filter((instance) -> (determineMouseRetire(instance))).forEach((instance) -> {
                            retireList.add(instance);
                    });
			
                        retireList.stream().map((instance) -> {
                            instance.retire();
                        return instance;
                    }).map((instance) -> {
                        ...
                        mouseInstances.stream().forEach((instance) -> {
                            try
                            {
                        ...
                        movableMouse.keySet().stream().forEach((instance) -> {
                            final MouseRepresent instanceF = instance;
                            final Grid gridF = movableMouse.get(instance);
                            
                            ExecutorService exe = Executors.newCachedThreadPool();
                            Callable<Integer> caller = () -> instanceF.getNextMove(gridF);
                            ...
                            try
                            {
                                ...
                                {
                                    ArrayList<MouseRepresent> rep = new ArrayList<>();
                                    rep.add(instance);
