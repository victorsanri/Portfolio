...
public class GameController
{
	...
	private final int gridSize;
        // Modified 2017-01-28: Several active cheeses
	// private Cheese cheese;
        private Cheese[] cheese;
	private final GameControllerAdapter adapter;
	private final ArrayList<MouseController> mouseList;
	private final ArrayList<Bomb> bombList;
	...
	public void start()
	{
		...
		Grid origin = maze.getGrid(0, 0);
                mouseList.stream().forEach((MouseController mouse) -> {
                    try
                    {
		...
		try
		{
				...
				(yOnField >= targetYOnGrid - leeway && yOnField <= targetYOnGrid + leeway))
			{
				...
				// Modified: 2017-01-28
                                ...
                                int pos = mouse.getNumberOfCheese(); // actually, next cheese to be eaten
                                if (this.cheese[pos].getX() == targetGrid.getX() && this.cheese[pos].getY() == targetGrid.getY())
                                {
					represent.displayCheeseNumber();
					mouse.increaseNumberOfCheese();
                                        mouse.getMouse().newCheese();
					this.numberOfCheeseTaken++;
					
					//if (this.numberOfCheeseTaken == this.numberOfCheese)
                                        // Game ends when first mouse gets all the cheese
                                        if (mouse.getNumberOfCheese() == this.numberOfCheese)
					{
						this.adapter.stop();
					}
				//	else
	...
	*/
	...
	{
		mouseList.clear();
                MouseLoader.load().stream().forEach((Class<?> clz) -> {
                    try {
                        ...
                        controller.setNumberOfBombs((int) (GameController.this.numberOfCheese * GameConfig.RATIO_BOMBS_TO_CHEESE));
	...
	// new location of the cheese.
        // To be modified: 2017-01-28
        // Instead of placing one cheese at one random grid in the maze, it must
        // place a number of cheeses all around the maze. They will not disappear
        // as they are eaten by mice. Hence, this method must be called only once.
	...
	//}
        ...
        {
                Random random = new Random();
                this.cheese = new Cheese[numberOfCheese];
                for (int i=0; i<this.cheese.length; i++)
                {
                    int x = random.nextInt(maze.getWidth());
                    int y = random.nextInt(maze.getHeight());
                    //System.out.println(x + ", " + y);
                    this.cheese[i] = new Cheese(x, y);
                }
                mouseList.stream().forEach((controller) -> {
                    controller.getMouse().newCheese();
            });
                adapter.newCheese(this.cheese);
        }       
